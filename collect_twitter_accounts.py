...
def check_location(user):
	...
	if city and state:
		get_county(state, city)

#attempt to get a county match based on the location provided by the user
def get_county(state, county, city=None):
	if city:
		query = """SELECT DISTINCT(county) FROM counties WHERE state ilike '%s' AND (primary_city ilike '%s' OR acceptable_cities ilike '%%%s%%')""" % (state, city, city)
		r = db.query(query)
	else: #check if the state and county combination exists
		query = """SELECT DISTINCT(county) FROM counties WHERE state ilike '%s' AND county ilike '%s'""" % (state, county)
		r = db.query(query)
	return r
...
def get_user_data(user):
	...
	return screen_name,name,uid,location
...
def get_followers(uid):
	results = api.followers_ids(uid)
	followers = []
	for follower in followers:
		followers.append(api.get_user(follower))
	return followers
